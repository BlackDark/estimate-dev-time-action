...
function filterDiffByPatterns(diffContent, ignorePatterns) {
    ...
    for (const section of fileSections) {
        ...
        if (ignorePatterns.length > 0 && shouldIgnoreFile(filename, ignorePatterns)) {
...
function categorizeFile(filename, analysis) {
    ...
    // Configuration files
    if (['json', 'yml', 'yaml', 'toml', 'ini', 'conf', 'config'].includes(extension) ||
        ['dockerfile', 'gitignore', 'gitattributes', 'editorconfig', 'prettierrc', 'eslintrc'].some(config => lowerFilename.includes(config)) ||
        ['package.json', 'tsconfig.json', 'jest.config.js', 'webpack.config.js', 'vite.config.js',
            'tailwind.config.js', 'next.config.js', 'babel.config.js', '.env'].some(config => lowerFilename.endsWith(config))) {
        analysis.configFiles.push(filename);
        return;
    }
    // Build/CI files
    if (lowerFilename.includes('.github/workflows/') ||
        lowerFilename.includes('/.github/') ||
        lowerFilename.includes('ci/') ||
        lowerFilename.includes('scripts/') ||
        ['makefile', 'dockerfile', 'docker-compose.yml'].some(build => lowerFilename.includes(build))) {
        analysis.buildFiles.push(filename);
        return;
    }
    // Documentation files
    if (['md', 'rst', 'txt', 'doc', 'docx', 'pdf'].includes(extension) ||
        ['readme', 'changelog', 'license', 'contributing', 'docs/'].some(doc => lowerFilename.includes(doc))) {
        analysis.documentationFiles.push(filename);
        return;
    }
    // Code files
    if (['js', 'ts', 'jsx', 'tsx', 'py', 'java', 'c', 'cpp', 'h', 'hpp', 'cs', 'php',
        'rb', 'go', 'rs', 'kt', 'swift', 'dart', 'vue', 'svelte', 'html', 'css', 'scss',
        'sass', 'less', 'sql', 'sh', 'bash', 'ps1', 'r', 'scala', 'clj', 'ex', 'exs'].includes(extension)) {
